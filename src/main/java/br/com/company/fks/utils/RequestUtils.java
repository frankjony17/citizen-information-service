...
public class RequestUtils {
    ...
    public ResponseEntity<?> doGet(String url, Class<?> clazz) {
        ...
        headers = headersUtils.getHeadersDefaults();
        if (headers == null) {
    ...
    public <T> ResponseEntity<?> doGetBetweenDomain(String url, Class<?> clazz, T t) {
        ...
        final HttpEntity<T> entity = new HttpEntity<T>(t, headers);

        return restTemplate.exchange(builder.build().encode().toUri(), HttpMethod.GET, entity, clazz);
    }

    /**
     * Efetua requisições HTTP do tipo POST entre os módulos FKSolutions passando o token no header
     *
     * @param url
     * @return
     */
     ...
     * @return
     */
    public ResponseEntity<?> doPost(String url, Object objeto) {
        HttpHeaders headers = headersUtils.getHeadersDefaults();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<?> entity = new HttpEntity<>(objeto, headers);
        return restTemplate.postForEntity(url, entity, objeto.getClass());
    }
    ...
    /**
     * Efetua requisições HTTP do tipo DELETE entre os módulos FKSolutions passando o token no header
     *
     * @param url
     * @return
     */
